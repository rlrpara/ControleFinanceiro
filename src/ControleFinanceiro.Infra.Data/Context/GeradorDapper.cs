...
using ControleFinanceiro.Infra.Data.Interfaces;
...
public class GeradorDapper : IGeradorDapper
{
    ...
    {
        if (!string.IsNullOrWhiteSpace(fullName) && fullName.Contains("Int32"))
        {
            return _parametrosConexao.TipoBanco switch
            {
        ...
        else if (!string.IsNullOrWhiteSpace(fullName) && fullName.Contains("DateTime"))
            return ObterParaData();
        else if (!string.IsNullOrWhiteSpace(fullName) && fullName.Contains("Boolean"))
            return "boolean";
        else
        {
            return _parametrosConexao.TipoBanco switch
    ...
    private string FormataValor<T>(PropertyInfo x, T entidade) where T : class
    {
        ...
        if (propriedade.Contains("string"))
            ...
            if (!string.IsNullOrWhiteSpace(x.PropertyType.FullName) && x.PropertyType.FullName.ToLower().Contains("datetime"))
                return $"{(string.IsNullOrWhiteSpace(x.GetValue(entidade)?.ToString()) ? "null" : $"'{Convert.ToDateTime(x.GetValue(entidade)):yyyy-MM-dd HH:mm:ss}'")}";
            else if (!string.IsNullOrWhiteSpace(x.PropertyType.FullName) && x.PropertyType.FullName.ToLower().Contains("string"))
    ...
    private void ObterConstraintSql<T>(StringBuilder sqlConstraint, Nota opcoesBase, string nomeCampo) where T : class
    {
        ...
        bool usarSchema = ObterNomeTabela<T>()?.Split(".").Length > 1;

        if (usarSchema)
            nomeChave = $"FK_{ObterNomeTabela<T>()?.Split(".")[1]}_{nomeCampo}".ToUpper();
        else
            nomeChave = $"FK_{ObterNomeTabela<T>()}_{nomeCampo}".ToUpper();
        ...
        switch (_parametrosConexao.TipoBanco)
        {
            case ETipoBanco.MySql:
                ...
                sqlConstraint.AppendLine($"REFERENCES {(usarSchema ? $"{_parametrosConexao.NomeBanco}.{tabelaChaveEstrangeira}" : $"{tabelaChaveEstrangeira}")} (ID) ON DELETE NO ACTION ON UPDATE NO ACTION;{Environment.NewLine}");
                break;
            ...
            case ETipoBanco.Postgresql:
                ...
                sqlConstraint.AppendLine($"REFERENCES {(usarSchema ? $"{_parametrosConexao.NomeBanco}.{tabelaChaveEstrangeira}" : $"{tabelaChaveEstrangeira}")} (ID);{Environment.NewLine}");
                break;
